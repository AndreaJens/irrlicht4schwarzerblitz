...
namespace irr
{
    struct SJoystickWin32Control
    {
		...
		core::array<core::stringc> ActiveJoysticksId;
    #endif
		...
		static BOOL CALLBACK CountJoysticks(LPCDIDEVICEINSTANCE lpddi, LPVOID cp);
        void directInputAddJoystick(LPCDIDEVICEINSTANCE lpddi);
		void directInputStoreAvailableJoystick(LPCDIDEVICEINSTANCE lpddi);
    #endif

        void pollJoysticks();
        bool activateJoysticks(core::array<SJoystickInfo> & joystickInfo);
		bool checkIfAllJoysticksAreActive();
		bool checkConnectedJoypads();
	...
	BOOL CALLBACK SJoystickWin32Control::CountJoysticks(LPCDIDEVICEINSTANCE lpddi, LPVOID cp)
	{
		SJoystickWin32Control* p = (SJoystickWin32Control*)cp;
		p->directInputStoreAvailableJoystick(lpddi);
		return DIENUM_CONTINUE;
	}

	void SJoystickWin32Control::directInputStoreAvailableJoystick(LPCDIDEVICEINSTANCE lpddi)
	{
		//Get the GUID of the joystuck
		core::stringc name = lpddi->tszProductName;
		ActiveJoysticksId.push_back(name);
	}
	...
	void SJoystickWin32Control::directInputAddJoystick(LPCDIDEVICEINSTANCE lpddi)
	{
		...
		// bug fixed for the Win32 mode (Device->HWnd => GetAncestor(Device->HWnd, GA_ROOT))
		if (FAILED(activeJoystick.lpdijoy->SetCooperativeLevel(GetAncestor(Device->HWnd, GA_ROOT), DISCL_BACKGROUND | DISCL_EXCLUSIVE)))
		{
			RECT clientRect;
			GetClientRect(Device->HWnd, &clientRect);
			int windowWidth = clientRect.right;
			int windowHeight = clientRect.bottom;
			char message[100];
			sprintf(message, "winW = %d winH = %d", windowWidth, windowHeight);
			os::Printer::log(message, ELL_WARNING);
...
bool SJoystickWin32Control::checkIfAllJoysticksAreActive() {
#ifdef _IRR_COMPILE_WITH_JOYSTICK_EVENTS_
#ifdef _IRR_COMPILE_WITH_DIRECTINPUT_JOYSTICK_
	//u32 size = ActiveJoysticksId.size();
	//ActiveJoysticksId.clear();
	//char logString[256];
	////sprintf(logString, "previously: %d joypads", size);
	//os::Printer::log(logString, ELL_INFORMATION);
	//DirectInputDevice->EnumDevices(DI8DEVCLASS_GAMECTRL, SJoystickWin32Control::CountJoysticks, this, DIEDFL_ATTACHEDONLY);
	//char logString2[256];
	////sprintf(logString2, "after: %d joypads", ActiveJoysticksId.size());
	//os::Printer::log(logString2, ELL_INFORMATION);
	//if (ActiveJoysticksId.size() != size) {
	//	return false;
	//}
	for (int joystick = 0; joystick < ActiveJoysticks.size(); ++joystick)
	{
		DIJOYSTATE2 info;
		if (FAILED(ActiveJoysticks[joystick].lpdijoy->GetDeviceState(sizeof(DIJOYSTATE2), &info)))
			return false;
	}
#endif
#endif
...
bool SJoystickWin32Control::checkConnectedJoypads() {
#ifdef _IRR_COMPILE_WITH_JOYSTICK_EVENTS_
#ifdef _IRR_COMPILE_WITH_DIRECTINPUT_JOYSTICK_
	u32 size = ActiveJoysticksId.size();
	ActiveJoysticksId.clear();
	...
	ExternalWindow(false), Win32CursorControl(0), JoyControl(0)
{
	...
	if (!CreationParams.WindowId && CreationParams.DriverType != video::EDT_NULL)
	{
		...
		if (CreationParams.Fullscreen || CreationParams.Borderless)
		{
			windowLeft = 0;
			windowTop = 0;
		}
...
void CIrrDeviceWin32::setWindowCaption(const wchar_t* text)
{
	...
	// FIX: HWnd => GetAncestor(HWnd, GA_ROOT)
	DWORD_PTR dwResult;
	SendMessageTimeoutW(GetAncestor(HWnd, GA_ROOT), WM_SETTEXT, 0,
...
{
	// Window active /focus bugfix
	//return (GetActiveWindow() == HWnd);
	bool ret = (GetActiveWindow() == GetAncestor(HWnd, GA_ROOT));
	return ret;
}
...
bool CIrrDeviceWin32::isWindowMinimized() const
{
	...
	// bug fix for focus
	if (GetWindowPlacement(GetAncestor(HWnd, GA_ROOT), &plc))
		ret |= (plc.showCmd & SW_SHOWMINIMIZED) != 0;
	return ret;
}
...
bool CIrrDeviceWin32::checkIfAllJoysticksAreActive()
{
	if (JoyControl)
		return JoyControl->checkIfAllJoysticksAreActive();
	else
		return true;
}

bool CIrrDeviceWin32::checkConnectedJoypads()
{
	if (JoyControl)
		return JoyControl->checkConnectedJoypads();
	else
		return true;
}
