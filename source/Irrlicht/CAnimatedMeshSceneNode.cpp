...
: IAnimatedMeshSceneNode(parent, mgr, id, position, rotation, scale), Mesh(0),
	...
	CurrentFrameHasBeenChanged(false)
{
	#ifdef _DEBUG
...
void CAnimatedMeshSceneNode::setCurrentFrame(f32 frame, bool resetAnimation)
{
	// if you pass an out of range value, we just clamp it
	CurrentFrameNr = core::clamp ( frame, (f32)StartFrame, (f32)EndFrame );
	if (resetAnimation) {
		CurrentFrameHasBeenChanged = true;
	}
...
IMesh * CAnimatedMeshSceneNode::getMeshForCurrentFrame()
{
	...
	else
	{
		...
		{
			// FIX proposed by Mel @ http://irrlicht.sourceforge.net/forum/viewtopic.php?f=7&t=47041
			// this patch slows down update a bit, but ensures that all the childs of the mesh are updated at the right time,
			// instead of having to wait an additional update. This should fix the issue with child objects on the mesh.
			for (u32 n = 0; n<JointChildSceneNodes.size(); ++n)
				if (JointChildSceneNodes[n]->getParent() == this)
				{
					JointChildSceneNodes[n]->updateAbsolutePositionOfAllChildren(); //temp, should be an option
...
{
	//setup time for frame update
	u32 deltaT = timeMs - LastTimeMs;
	if (CurrentFrameHasBeenChanged) {
		deltaT = 0;
		LastTimeMs = 0;
		CurrentFrameHasBeenChanged = false;
	}
	if (InternalTimerUpdateMultiplier != 1.f) {
		deltaT = (u32)(round((f32)deltaT * InternalTimerUpdateMultiplier));
	}
	...
	// WARNING: dirty fix done dirt cheap to try to prevent the engine to recalculate unnecessary transformations (i.e. multiple OnAnimate calls)
	if (deltaT != 0) {
		// set CurrentFrameNr
		buildFrameNr(deltaT);
