...
IImage* CD3D9Driver::createScreenShot(video::ECOLOR_FORMAT format, video::E_RENDER_TARGET target)
{
	...
	RECT clientRect;
	{
		...
		clientPoint.x = 0;
		clientPoint.y = 0;
		HMONITOR monitor = MonitorFromWindow((HWND)getExposedVideoData().D3D9.HWnd, MONITOR_DEFAULTTOPRIMARY);
		DEVICE_SCALE_FACTOR scale;
		/*
		HRESULT result = GetScaleFactorForMonitor(monitor, &scale);
		...
		*/
		...
		clientRect.left   = irr::core::floor32(scaleFactor * clientPoint.x);
		clientRect.top	= irr::core::floor32(scaleFactor * clientPoint.y);
		clientRect.right  = irr::core::floor32(scaleFactor * (clientRect.left + ScreenSize.Width));
		clientRect.bottom = irr::core::floor32(scaleFactor * (clientRect.top  + ScreenSize.Height));
	...
	shotSize.Width = core::min_((u32)irr::core::floor32(scaleFactor * ScreenSize.Width), (u32)(clientRect.right-clientRect.left) );
	shotSize.Height = core::min_((u32)irr::core::floor32(scaleFactor * ScreenSize.Height), (u32)(clientRect.bottom-clientRect.top) );
